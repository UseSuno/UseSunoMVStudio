...
import { ArrowLeft, Check, FileAudio, RotateCcw, Save, TimerReset } from 'lucide-react';
...
export function TimestampApp() {
  ...
  useEffect(() => {
    ...
    void loadLocal().then(value => {
      ...
      setSaved(value); setProject(value.project); const next = tokensFromLines(value.project.lines); setTokens(next);
  ...
  const save = async () => {
    ...
    const lines = applyTokenTimes(project.lines, tokens, project.duration);
  ...
  return <main className="timestamp-page"><header className="timestamp-header"><a href="/"><ArrowLeft size={17}/>返回 MV Studio</a><div><span>VERSE / TIMING</span><h1>逐字歌词打点</h1></div><button className="primary" disabled={!project} onClick={() => void save()}><Save size={15}/>保存到工程</button></header>
