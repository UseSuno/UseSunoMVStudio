...
export function FoliaStage({ project, peaks, clock, fontBlob }: { project: Project; peaks: number[]; clock: PlaybackClock; fontBlob: Blob | null }) {
  ...
  useEffect(() => {
    const listener = (e: MessageEvent) => { if (e.origin !== location.origin || e.source !== frame.current?.contentWindow) return;
      if (e.data?.type === 'verse:ready') { ready.current = true; frame.current?.contentWindow?.postMessage({ type: 'verse:project', project: latest.current.project }, location.origin); if (fontBlob && latest.current.project.font === 'local') void fontBlob.arrayBuffer().then(buffer => frame.current?.contentWindow?.postMessage({ type: 'verse:font', family: latest.current.project.customFontName || 'Verse Local Font', buffer }, location.origin, [buffer])); }
