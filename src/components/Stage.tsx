...
export function Stage({ project, peaks, audioAnalysis, clock, fontBlob, coverBlob, monetPortraitBlob, onError }: { project: Project; peaks: number[]; audioAnalysis: AudioAnalysis; clock: PlaybackClock; fontBlob: Blob | null; coverBlob: Blob | null; monetPortraitBlob: Blob | null; onError: (s: string) => void }) {
  ...
  return <section className="stage-area">
    <div className="stage-toolbar"><span><i className="status-dot" /> {t('stage.live')}</span><span>{project.ratio} <span className="dim">/</span> {t('stage.fit')}</span><button className="icon-button" data-tooltip={t('stage.fullscreen')} aria-label={t('stage.fullscreen')} onClick={() => { void stage.current?.requestFullscreen().catch(e => onError(e.message)); }}><Maximize2 size={15}/></button></div>
