...
export function ExportModal({ project, buffer, peaks, close, clock, artifact, setArtifact }: { project: Project; buffer: AudioBuffer | null; peaks: number[]; clock: PlaybackClock; close: () => void; artifact: ExportArtifact | null; setArtifact: (artifact: ExportArtifact | null) => void }) {
  ...
  return <Modal title="导出歌词影片" close={close} busy={running}><div className="export-summary"><Film size={26}/><div><strong>{project.title}</strong><p>{w} × {h} · 30 fps · 包含音频</p></div></div>
    <fieldset disabled={running}><div className="form-row"><label>分辨率<select value={height} onChange={e => setHeight(Number(e.target.value))}><option value={720}>720p · 标准</option><option value={1080}>1080p · 高清</option></select></label><label>视频格式<select value={format} onChange={e => setFormat(e.target.value as 'mp4' | 'webm')}><option value="mp4" disabled={!support?.mp4}>MP4 {support && !support.mp4 ? '· 不支持' : ''}</option><option value="webm" disabled={!support?.webm}>WebM {support && !support.webm ? '· 不支持' : ''}</option></select></label></div>
    <label className="form-label">导出方式<select value={mode} onChange={e => setMode(e.target.value as typeof mode)}><option value="offline" disabled={project.template.startsWith('folia-') && !fastFolia}>快速逐帧 · 无需完整播放</option><option value="realtime">{project.template.startsWith('folia-') ? '兼容录制 · 保留原始动画' : '兼容录制 · 需要完整播放'}</option></select></label>
    <div className="form-row"><label>开始时间 / 秒<input type="number" min="0" step="0.1" value={range.start} onChange={e => setRange({ ...range, start: Number(e.target.value) })}/></label><label>结束时间 / 秒<input type="number" min="0" step="0.1" max={project.duration} value={range.end} onChange={e => setRange({ ...range, end: Number(e.target.value) })}/></label></div></fieldset>
    ...
    {artifact && <><VideoResult blob={artifact.blob}/><p className="export-retained">返回编辑后仍会保留在当前 Studio 会话中；刷新或关闭页面后清除。</p></>}
    <div className="modal-actions"><div>{running ? <button onClick={() => controller.current?.abort()}>取消导出</button> : <button onClick={close}>返回编辑</button>}{artifact && !running ? <><button className="discard-export" onClick={() => setArtifact(null)}>丢弃成品</button><button disabled={!buffer || !support?.[format] || untimed || outOfRange || !validRange} onClick={() => void start()}>重新导出</button></> : null}</div>{artifact && !running ? <button className="primary" onClick={() => downloadBlob(artifact.blob, artifact.filename)}><ArrowDownToLine size={15}/>下载视频</button> : <button className="primary" disabled={running || !buffer || !support?.[format] || untimed || outOfRange || !validRange} onClick={() => void start()}>{running ? <Loader2 className="spin" size={15}/> : <ArrowDownToLine size={15}/>} {running ? '正在导出' : '开始导出'}</button>}</div>
  </Modal>;
}
