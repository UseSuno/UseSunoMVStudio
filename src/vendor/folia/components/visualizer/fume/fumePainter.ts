...
export const createFumeLayoutTiming = (): FumeLayoutAttemptTiming => ({
    lines: 0,
    prepareLayoutMs: 0,
    placementMs: 0,
    renderDetailsMs: 0,
...
export let lastFumePassedFadeDurationCache: {
    key: string;
    duration: number;
} | null = null;
...
export const resolveWordRevealProgress = (
    range: WordRange,
    currentTimeValue: number,
) => {
...
export const resolvePrintedGlyphsInRange = (
    range: WordRange,
    currentTimeValue: number,
) => {
...
export const hasRevealCompletedByLineEnd = (
    line: Line,
    currentTimeValue: number,
...
export let segmentMeasureCanvas: HTMLCanvasElement | null = null;
export const segmentMeasureCache = new Map<string, number[]>();
...
export const measureSegmentGlyphOffsets = (
    ...
    if (typeof document === 'undefined' && typeof OffscreenCanvas === 'undefined') return offsets;

    if (!segmentMeasureCanvas) {
        segmentMeasureCanvas = (typeof document === 'undefined' ? new OffscreenCanvas(1, 1) : document.createElement('canvas'));
    }
...
export const createStaticBlockSnapshot = (
    ...
    const rasterScale = clamp((typeof window === 'undefined' ? workerPixelRatio : window.devicePixelRatio) || 1, 1, 2);
...
let workerPixelRatio = 1;
export function setFumeWorkerPixelRatio(value: number) { workerPixelRatio = value; }

export function createFumePainter(options: any) {
 const { resolvedFumeTuning, lines, article, viewport, theme, currentTime, audioPower, audioBands, cameraInitializedRef, cameraRef, cameraRetargetRef, currentLineIndexRef, staticBlockSnapshotCacheRef, hasPrintedContentRef, setHasPrintedContent, backgroundScene, staticMode, backgroundObjectOpacity, showText, overviewCamera, overviewStartTime, cameraSpeed, glowIntensity, passedFadeDuration, showPrintStamp, textHoldRatio } = options;
        ...
        const dpr = (typeof window === 'undefined' ? options.dpr : window.devicePixelRatio) || 1;

        if (canvas.width !== Math.floor(width * dpr) || canvas.height !== Math.floor(height * dpr)) {
            canvas.width = Math.floor(width * dpr);
            canvas.height = Math.floor(height * dpr);
            if (canvas.style) canvas.style.width = `${width}px`;
        ...
        const draw = (now: number) => {
            ...
            const currentDpr = (typeof window === 'undefined' ? options.dpr : window.devicePixelRatio) || 1;
            ...
            if (canvas.width !== Math.floor(currentWidth * currentDpr) || canvas.height !== Math.floor(currentHeight * currentDpr)) {
                ...
                if (canvas.style) canvas.style.width = `${currentWidth}px`;
                if (canvas.style) canvas.style.height = `${currentHeight}px`;
            }
            ...
            if (showText) {
                ...
                if (staticState) {
                    const snapshotScale = clamp((typeof window === 'undefined' ? options.dpr : window.devicePixelRatio) || 1, 1, 2);
 ...
 if (typeof document !== 'undefined' && options.register !== false) {
   activePainter = options; painterVersion++;
   if (externalPainting) pendingSteps = [];
   options.canvas.dataset.fumePainter = '';
 }
 return { draw(now: number) {
   if (typeof document !== 'undefined' && options.register !== false && externalPainting) {
     const time = currentTime.get();
     if (!hasPrintedContentRef.current && article && time >= article.firstRenderableStartTime) {
       hasPrintedContentRef.current = true; setHasPrintedContent(true);
     }
     pendingSteps.push({ now, time, index: currentLineIndexRef.current,
       audio: Object.fromEntries(['bass','lowMid','mid','vocal','treble'].map(key => [key, audioBands[key].get()]).concat([['power', audioPower.get()]])) });
     return;
   }
   draw(now);
 }, dispose() {
   if (activePainter === options) { activePainter = undefined; pendingSteps = []; sentVersion = -1; }
 } };
}

let externalPainting = false;
let activePainter: any;
let painterVersion = 0, sentVersion = -1;
let pendingSteps: any[] = [];
export function setFumeExternalPainting(enabled: boolean) {
  externalPainting = enabled; pendingSteps = []; sentVersion = -1;
}
export function takeFumeWorkerFrame() {
  if (!activePainter) throw new Error('Fume painter is not ready.');
  const options = activePainter;
  let snapshot: any;
  if (sentVersion !== painterVersion) {
    const omit = new Set(['canvas', 'currentTime', 'audioPower', 'audioBands', 'setHasPrintedContent', 'staticBlockSnapshotCacheRef']);
    snapshot = Object.fromEntries(Object.entries(options).filter(([key]) => !omit.has(key)));
    // Snapshot caches are local to each renderer, never transferred.
    snapshot = structuredClone(snapshot);
    snapshot.dpr = window.devicePixelRatio || 1;
    const measure = options.canvas.getContext('2d');
    measure.save();
    snapshot.fontProbes = (options.article?.blocks ?? []).map(block => {
      const font = buildCanvasFont(block, options.theme); measure.font = font;
      return { font, text: block.line.fullText, width: measure.measureText(block.line.fullText).width };
    });
    measure.restore();
    sentVersion = painterVersion;
  }
  const steps = pendingSteps; pendingSteps = [];
