...
export const captureCopy: Record<string, CaptureCopy> = {
  en: {
    ...
    notes: ['Captures the original canvas surfaces and text separately, then composites and encodes them in parallel. Available to try for all templates; speed varies by scene. If a scene cannot be preserved, use real-time recording. The preview is hidden during export and restored afterward.', 'Requires a recent Chromium browser with {flag} enabled and a browser restart. Unsupported or failed capture falls back to Standard capture.', 'Available only for Fume with the Fluid light background. Composites the original canvas surfaces directly. No browser flags required.', 'Available only for Fume with the Fluid light background. Runs canvas lyric rendering and encoding on a separate thread. No browser flags required.'],
  },
  'zh-CN': {
    ...
    notes: ['分别捕获原始画布和文字，再并行合成与编码。所有模板均可尝试，速度因画面而异；无法完整保留的画面请改用实时录制。导出期间暂时隐藏预览，结束后恢复。', '需要新版 Chromium 浏览器开启 {flag} 并重启浏览器。不支持或捕获失败时自动回退标准画面捕获。', '仅适用于浮名搭配流体织光背景，直接合成原始画布画面，无需开启浏览器实验开关。', '仅适用于浮名搭配流体织光背景，在独立线程中绘制画布歌词并编码，无需开启浏览器实验开关。'],
  },
  'zh-TW': {
    ...
    notes: ['分別擷取原始畫布與文字，再平行合成與編碼。所有範本均可嘗試，速度依畫面而異；無法完整保留的畫面請改用即時錄製。匯出時暫時隱藏預覽，結束後恢復。', '需要新版 Chromium 瀏覽器啟用 {flag} 並重新啟動。不支援或擷取失敗時自動退回標準畫面擷取。', '僅適用於浮名搭配流體織光背景，直接合成原始畫布，不需啟用瀏覽器實驗開關。', '僅適用於浮名搭配流體織光背景，在獨立執行緒繪製畫布歌詞並編碼，不需啟用瀏覽器實驗開關。'],
  },
  id: {
    ...
    notes: ['Merekam kanvas asli dan teks secara terpisah, lalu menggabungkan dan mengodekannya secara paralel. Dapat dicoba pada semua templat; kecepatannya bergantung pada adegan. Gunakan perekaman waktu nyata jika adegan tidak dapat dipertahankan. Pratinjau disembunyikan selama ekspor dan dipulihkan setelahnya.', 'Memerlukan Chromium terbaru, mengaktifkan {flag}, lalu memulai ulang browser. Jika tidak didukung atau gagal, perekaman standar digunakan.', 'Hanya untuk Fume dengan latar Cahaya mengalir. Menggabungkan kanvas asli secara langsung tanpa mengaktifkan fitur eksperimental browser.', 'Hanya untuk Fume dengan latar Cahaya mengalir. Menggambar lirik kanvas dan mengodekan video pada thread terpisah tanpa fitur eksperimental browser.'],
  },
  hi: {
  ...
  de: {
    ...
    notes: ['Erfasst Original-Canvas und Text getrennt und verarbeitet Komposition und Kodierung parallel. Für alle Vorlagen ausprobierbar; die Geschwindigkeit hängt von der Szene ab. Wenn eine Szene nicht erhalten werden kann, nutze die Echtzeitaufnahme. Die Vorschau wird während des Exports ausgeblendet und danach wiederhergestellt.', 'Erfordert einen aktuellen Chromium-Browser, die Aktivierung von {flag} und einen Browserneustart. Nicht unterstützte oder fehlgeschlagene Aufnahmen nutzen die Standardmethode.', 'Nur für Fume mit Fließendes-Licht-Hintergrund. Setzt die Original-Canvas direkt zusammen. Keine experimentellen Browsereinstellungen nötig.', 'Nur für Fume mit Fließendes-Licht-Hintergrund. Zeichnet Canvas-Liedtext und kodiert in einem separaten Thread. Keine experimentellen Browsereinstellungen nötig.'],
  },
  ur: {
    ...
    notes: ['اصل کینوس اور متن کو الگ کیپچر کرکے متوازی طور پر جوڑتا اور انکوڈ کرتا ہے۔ تمام ٹیمپلیٹس میں آزما سکتے ہیں؛ رفتار منظر کے مطابق بدلتی ہے۔ منظر محفوظ نہ رہے تو ریئل ٹائم ریکارڈنگ استعمال کریں۔ ایکسپورٹ کے دوران پیش نظارہ چھپتا ہے اور بعد میں بحال ہوتا ہے۔', 'نیا Chromium براؤزر، {flag} کو فعال کرنا اور براؤزر دوبارہ شروع کرنا ضروری ہے۔ عدم معاونت یا ناکامی پر معیاری کیپچر استعمال ہوتا ہے۔', 'صرف Fume اور بہتی روشنی پس منظر کے لیے۔ اصل کینوس براہ راست جوڑتا ہے؛ براؤزر کی تجرباتی ترتیبات ضروری نہیں۔', 'صرف Fume اور بہتی روشنی پس منظر کے لیے۔ الگ تھریڈ میں کینوس کے بول بناتا اور انکوڈ کرتا ہے؛ براؤزر کی تجرباتی ترتیبات ضروری نہیں۔'],
  },
  ru: {
    ...
    notes: ['Захватывает исходные canvas и текст отдельно, затем выполняет композицию и кодирование параллельно. Можно попробовать со всеми шаблонами; скорость зависит от сцены. Если сцену нельзя сохранить, используйте запись в реальном времени. Предпросмотр скрывается на время экспорта и восстанавливается после него.', 'Нужен современный Chromium с включённым {flag} и перезапуск браузера. При отсутствии поддержки или ошибке используется стандартный захват.', 'Только для Fume с фоном «Струящийся свет». Напрямую объединяет исходные canvas. Экспериментальные настройки браузера не нужны.', 'Только для Fume с фоном «Струящийся свет». Рисует текст на canvas и кодирует в отдельном потоке. Экспериментальные настройки браузера не нужны.'],
  },
  vi: {
    ...
    notes: ['Chụp canvas gốc và văn bản riêng, rồi tổng hợp và mã hóa song song. Có thể thử với mọi mẫu; tốc độ tùy theo cảnh. Nếu không thể giữ nguyên cảnh, hãy dùng ghi hình thời gian thực. Bản xem trước được ẩn khi xuất và khôi phục sau đó.', 'Cần Chromium mới, bật {flag} rồi khởi động lại trình duyệt. Nếu không được hỗ trợ hoặc chụp thất bại, sẽ dùng phương thức tiêu chuẩn.', 'Chỉ dành cho Fume với nền Ánh sáng chảy. Tổng hợp trực tiếp các canvas gốc, không cần bật tính năng thử nghiệm của trình duyệt.', 'Chỉ dành cho Fume với nền Ánh sáng chảy. Vẽ lời bài hát trên canvas và mã hóa trong luồng riêng, không cần bật tính năng thử nghiệm của trình duyệt.'],
  },
};
