...
export async function recordVideo(project: Project, buffer: AudioBuffer, peaks: number[], options: ExportOptions, signal: AbortSignal, progress: (p: number, text: string) => void, clock?: PlaybackClock) {
  ...
  try {
    if (signal.aborted) throw new DOMException(exportMessage('canceled'), 'AbortError');
    if (clock) { clock.pause(); clock.exporting = true; clock.time = options.start; }
    ...
    const tick = () => { const elapsed = audioContext.currentTime - start; if (clock) clock.time = options.start + Math.min(elapsed, duration); drawFrame(context, plan, options.start + Math.min(elapsed, duration)); progress(Math.min(1, elapsed / duration), exportMessage('recording')); if (elapsed >= duration) stop(); else frame = requestAnimationFrame(tick); };
    tick(); return await finished;
  } finally { if (clock) { clock.exporting = false; clock.seek(oldTime!); } cancelAnimationFrame(frame); signal.removeEventListener('abort', abort); document.removeEventListener('visibilitychange', hidden); try { source.stop(); } catch { /* Source may already have ended. */ } stream.getTracks().forEach(t => t.stop()); await audioContext.close(); canvas.width = 0; }
}
