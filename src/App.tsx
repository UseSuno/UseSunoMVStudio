...
export default function App() {
  ...
  const [audio, setAudio] = useState<Blob | null>(null), [fontBlob, setFontBlob] = useState<Blob | null>(null), [coverBlob, setCoverBlob] = useState<Blob | null>(null), [monetPortraitBlob, setMonetPortraitBlob] = useState<Blob | null>(null), [buffer, setBuffer] = useState<AudioBuffer | null>(null), [peaks, setPeaks] = useState<number[]>([]), [audioAnalysis, setAudioAnalysis] = useState<AudioAnalysis>(() => emptyAudioAnalysis());
  ...
  return <div className="studio" onDragOver={e => { e.preventDefault(); if (!busy && !modal) setDragging(true); }} onDragLeave={e => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setDragging(false); }} onDrop={e => { e.preventDefault(); setDragging(false); if (!busy && !modal) void handleFiles(Array.from(e.dataTransfer.files)); }}>
    ...
    {modal === 'export' && <ExportModal clock={clock} project={project} buffer={buffer} peaks={peaks} audioAnalysis={audioAnalysis} artifact={exportArtifact} setArtifact={setExportArtifact} close={() => setModal(null)}/>}
