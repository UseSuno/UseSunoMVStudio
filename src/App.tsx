...
export default function App() {
  ...
  const [audio, setAudio] = useState<Blob | null>(null), [fontBlob, setFontBlob] = useState<Blob | null>(null), [buffer, setBuffer] = useState<AudioBuffer | null>(null), [peaks, setPeaks] = useState<number[]>([]);
  ...
  const audioInput = useRef<HTMLInputElement>(null), lyricsInput = useRef<HTMLInputElement>(null), projectInput = useRef<HTMLInputElement>(null), fontInput = useRef<HTMLInputElement>(null), temperaInput = useRef<HTMLInputElement>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try { const decoded = blob ? await decodeAudio(blob) : null; if (!alive) return; if (saved?.font) { await installLocalFont(saved.font, p.customFontName || 'Verse Local Font'); setFontBlob(saved.font); } replace(p); attach(blob, decoded, p.duration); if (!saved) clock.seek(13.5); select(p.lines[2]?.id ?? p.lines[0]?.id ?? null); }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => { void saveLocal({ project, audio, font: fontBlob }).then(() => { if (alive) setSaveStatus('已保存到本机'); }).catch(() => { if (alive) setSaveStatus('保存失败 · 请下载工程'); }); }, 650);
    return () => { alive = false; clearTimeout(timer); };
  }, [project, audio, fontBlob, ready]);
  ...
  const importAudio = async (file: File) => {
    ...
    try {
      ...
      replace({ ...previous, title: metadata?.title || file.name.replace(/\.[^.]+$/, ''), artist: metadata?.artist || '', duration: decoded.duration, audioName: file.name, ...(previous.source === demoSource && !previous.audioName ? { lines: [], source: '' } : {}) });
  ...
  const handleFiles = async (files: File[]) => {
    try {
      ...
      if (archive) { setBusy('正在打开工程…'); const saved = await readProject(archive); const blob = saved.audio ?? (!saved.project.audioName ? demoAudio() : null); const decoded = blob ? await decodeAudio(blob) : null; if (saved.font) await installLocalFont(saved.font, saved.project.customFontName || 'Verse Local Font'); setFontBlob(saved.font ?? null); replace(saved.project); attach(blob, decoded, saved.project.duration); setCandidates([]); notify(blob ? '工程与素材已恢复。' : '工程已恢复，请重新关联音频文件。'); }
  ...
  const openTimestamp = () => {
    clock.pause();
    void saveLocal({ project: current.current, audio, font: fontBlob })
  ...
  return <div className="studio" onDragOver={e => { e.preventDefault(); if (!busy && !modal) setDragging(true); }} onDragLeave={e => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setDragging(false); }} onDrop={e => { e.preventDefault(); setDragging(false); if (!busy && !modal) void handleFiles(Array.from(e.dataTransfer.files)); }}>
    ...
    <div className="workspace"><aside className="left-panel"><div className="source-heading"><span><Music2 size={15}/>音频素材</span><button aria-label="导入音频" title="导入音频" onClick={() => audioInput.current?.click()}><Upload size={14}/></button></div><button className="audio-card" onClick={() => audioInput.current?.click()}><span className="audio-cover"><AudioLines size={25}/></span><span><strong>{project.audioName || (audio ? '把日子写成诗' : '导入一首歌曲')}</strong><small>{audio ? `${Math.floor(project.duration / 60)}:${Math.floor(project.duration % 60).toString().padStart(2, '0')} · ${project.audioName ? '本地音频' : '演示环境音'}` : 'MP3 / WAV / M4A / FLAC'}</small></span><ChevronDown size={13}/></button>
      {audio && project.audioName && !candidates.length && <button className="reread-lyrics" onClick={() => { setBusy('正在读取内嵌歌词…'); void embeddedLyrics(audio).then(m => { setCandidates(m.candidates); if (!m.candidates.length) notify('音频中未找到歌词。'); }).catch(e => notify(e.message)).finally(() => setBusy('')); }}>重新读取内嵌歌词</button>}
      {!!candidates.length && <div className="embedded"><span>发现内嵌歌词</span>{candidates.map((c, i) => <button key={i} onClick={() => { try { applyLyrics(c.text); setCandidates([]); } catch (err) { notify((err as Error).message); } }}>{c.label} <span>使用 →</span></button>)}</div>}
    ...
    {modal === 'save' && <Modal title="保存创作工程" close={() => setModal(null)}><p className="modal-description">下载可继续编辑的 .lyricmv 工程。画幅、动画参数、歌词校时和本地字体都会保留。</p><label className="toggle-label package-toggle"><span>包含音频素材 <small>在其他设备上继续创作</small></span><input type="checkbox" checked={includeAudio} onChange={e => setIncludeAudio(e.target.checked)}/></label><div className="modal-actions"><button onClick={() => downloadBlob(new Blob([toLrc(project.lines, project.offset)], { type: 'text/plain;charset=utf-8' }), `${project.title}.lrc`)}>仅下载 LRC</button><button className="primary" onClick={() => { void downloadProject(project, audio, includeAudio, fontBlob).then(() => { setModal(null); notify('工程已下载。'); }).catch(e => notify(e.message)); }}><ArrowDownToLine size={15}/>下载工程</button></div></Modal>}
  </div>;
}
